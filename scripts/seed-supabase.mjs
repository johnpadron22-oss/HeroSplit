#!/usr/bin/env node
/**
 * HeroSplit Workout Seeder (Supabase)
 *
 * Inserts or updates all workout programs in the Supabase `workouts` table.
 * Uses the service role key (bypasses RLS — the workouts table has no client
 * INSERT/UPDATE policy by design).
 *
 * Run:
...
// Converts a camelCase workout object (from old seed) to snake_case for Supabase
function toRow(w) {
  return {
    slug:          w.slug,
    name:          w.name,
    ...
    avatar_emoji:  w.avatarEmoji ?? null,
    equipment:     w.equipment ?? null,
    series:        w.series ?? null,
    workout_style: w.workoutStyle ?? null,
    is_pro:        Boolean(w.isPro),
  };
}
...
async function seed() {
  ...
  // Fetch existing slugs so we can report creates vs updates
  ...
  for (const workout of workouts) {
    const row = toRow(workout);
    if (DRY_RUN) {
      const action = existingSlugs.has(workout.slug) ? "update" : "create";
      console.log(`  [dry-run] Would ${action}: ${workout.name}`);
    ...
    if (error) {
      console.error(`  ❌ Failed to upsert ${workout.slug}: ${error.message}`);
      continue;
    }
