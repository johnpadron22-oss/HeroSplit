  ...
  return (
    <div className="min-h-screen bg-background text-foreground relative overflow-hidden">
      ...
      <nav className="relative z-10 px-6 py-6 flex justify-between items-center max-w-7xl mx-auto">
        ...
        <Button variant="outline" className="rounded-full px-6 hover:bg-white/5" onClick={loginWithGoogle}>
          Login
        </Button>
      </nav>
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-6 pt-16 pb-32">
        <motion.div variants={containerVariants} initial="hidden" animate="visible" className="text-center max-w-3xl mx-auto">
          ...
          <motion.p variants={itemVariants} className="text-lg md:text-xl text-muted-foreground mb-10 max-w-xl mx-auto leading-relaxed">
            From "100 Pushups" to "God Level" intensity. Unlock themed workout plans, track your stats, and build your physique.
          </motion.p>

          <motion.div variants={itemVariants} className="flex flex-col sm:flex-row gap-4 justify-center items-center">
            <Button size="lg" className="h-14 px-8 rounded-full text-lg bg-white text-black hover:bg-gray-200 transition-all shadow-[0_0_20px_rgba(255,255,255,0.3)]" onClick={loginWithGoogle}>
              Start Training <ArrowRight className="ml-2 w-5 h-5" />
            </Button>
            <Button variant="ghost" className="h-14 px-8 rounded-full text-lg hover:bg-white/5">View Workouts</Button>
          </motion.div>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6, duration: 0.8 }} className="grid md:grid-cols-3 gap-6 mt-32">
          {[
